---
name: import
description: "Import intelligent par passes intentionnelles : alignement profond, scan LS par l'agent principal, délégation à des sous-agents briefés, plan mode puis build mode, feedback à la fin. Tout atterrit dans le Cerveau Privé ; ce qui est collectif part ensuite vers un cerveau partagé via /partager."
---
...
## Où atterrit ce que tu importes

Deux questions de routage, dans cet ordre. Elles s'appliquent à **chaque item**, pas au lot.

**1. Est-ce utile à mémoriser ?**

Non (code, build, export, mockup, capture de debug, binaire) : ça va dans `Atelier/`, hors des cerveaux. Ça ne rentre pas dans le pipeline de classement.
Oui (note, décision, cadrage, livrable, compte-rendu, fiche) : question 2.

**2. Qui doit pouvoir le lire ?**

Chaque note appartient au cerveau de son audience. Les cerveaux disponibles sont les dossiers présents sous `Cerveaux/` ; la règle de routage par audience est dans le `AGENTS.md` racine. S'il n'y a que le `Cerveau Privé`, tout y va.

### Les invariants de l'import

**Tout arrive en Privé.** Un import écrit **toujours** dans le `Cerveau Privé`, sans exception. C'est le défaut, y compris quand tu hésites. Dans le doute, Privé, et tu ne poses même pas la question.

**Un import n'écrit jamais dans un cerveau partagé.** Ni toi, ni un sous-agent, ni un lot, jamais. Un cerveau partagé se remplit uniquement via le skill `/partager`, qui lit chaque note et contrôle la confidentialité (secrets, salaires, finances, RH nominative). Un import de masse qui écrirait direct dans un dossier partagé, c'est une fuite.

**Une note vit dans un seul cerveau.** Aucune copie entre cerveaux. Partager, c'est déplacer.

**Le partage est une deuxième passe.** Tu importes en Privé (étapes 1 à 8), puis tu proposes les candidats au partage (étape 8 bis), qui passent par `/partager`, note par note.

Donc : toutes les `destination` que tu produis pendant l'import sont sous `Cerveaux/Cerveau Privé/`. Ce que tu détectes de collectif, tu le **marques**, tu ne le déplaces pas.
...
5. Ce lot est-il un lot d'entreprise ?
   (des process, des fiches clients, de la doctrine, de la doc métier
    manifestement destinés à l'équipe, pas à toi seul)
   Si oui, je l'importe quand même en Privé, puis je te proposerai
   à la fin de le pousser vers le cerveau partagé qui convient via
   `/partager`, note par note. Rien ne part dans un cerveau partagé
   sans relecture.

6. As-tu un budget tokens limité ? Si oui, je peux router le scan
   via une clé OpenRouter DeepSeek V4 Flash (~10x moins cher).
```

Ne pose la question 5 que si elle a du sens (un lot pro, une source d'entreprise). Pour un export Apple Notes perso, ne la pose pas : c'est du Privé, point.
...
### 2.0 Écarter ce qui n'est pas à mémoriser

Première question de routage, appliquée au brut. Détecte ce qui n'a rien à faire dans un cerveau :

```bash
find "Cerveaux/Cerveau Privé/_Import/" -type f \( -iname "*.zip" -o -iname "*.dmg" -o -iname "*.psd" -o -iname "*.sketch" -o -iname "*.mp4" -o -iname "*.exe" \) -o -type d \( -name "node_modules" -o -name ".git" -o -name "dist" -o -name "build" \) 2>/dev/null
```

Code source, builds, exports d'outil, mockups, gros binaires, captures de debug : ça va dans `Atelier/`, hors des cerveaux. Liste ce que tu as trouvé, propose le déplacement, exécute après un OK. Ces items ne passent pas dans le scan.

Les archives (`.zip`) qui contiennent un export de notes font exception : décompresse-les, traite le contenu, écarte l'archive.
...
```bash
find "Cerveaux/Cerveau Privé/_Import/" -type f \( -iname "*.pdf" -o -iname "*.docx" -o -iname "*.pptx" -o -iname "*.xlsx" \) 2>/dev/null
```
...
3. **Convertis chaque fichier** en `.md` à côté :
   ...
   ```
4. **Déplace les originaux** dans `Cerveaux/Cerveau Privé/_Import/_originaux/`.
...
```bash
find "Cerveaux/Cerveau Privé/_Import/" -maxdepth 2 -type f -name "*.json" 2>/dev/null
```
...
2. **Pour les conversations**, garde en tête ces règles spécifiques :
   ...
   - Une conversation IA reste **privée par défaut**, même quand elle parle du boulot. C'est un brouillon de pensée, pas un document d'entreprise. Elle n'est jamais candidate au partage telle quelle.
3. **Parse et découpe** : crée **un fichier `.md` par conversation** dans `Cerveaux/Cerveau Privé/_Import/_chats/` :
...
- [A] items écartés vers `Atelier/` (code, builds, binaires)

Total à traiter dans cette passe : [X+Y+Z] items.
Destination de l'import : `Cerveaux/Cerveau Privé/`.
```
...
1. **Liste complète** :
   ```bash
   ls -laR "Cerveaux/Cerveau Privé/_Import/" | head -200
   ```
...
3. **Produis un mapping interne** :
   ...
   - Nature : [perso / pro individuel / collectif (process, fiches clients, doctrine)]
   - Lien avec ce que l'utilisateur a annoncé en Étape 1 : [match / écart]
   ```

   La ligne « Nature » sert à anticiper l'étape 8 bis. Si le corpus est manifestement collectif, dis-le maintenant, ça ne change rien à la destination d'import (Privé), ça prépare la proposition de partage.
...
Pour pouvoir briefer les sous-agents, tu dois maîtriser le vault. Tu lis le **Cerveau Privé** : c'est là que l'import atterrit.

1. **Contexte perso :** `Cerveaux/Cerveau Privé/2 CASQUETTES/Sur ma vie/Moi.md`
2. **Phase active :**
   ```bash
   ls -d "Cerveaux/Cerveau Privé/2 CASQUETTES/Sur ma vie/Life Phases/"*/ | sort -V | tail -1
   ```
   Lis son `[N] Intention.md`.
3. **Projets actifs :** liste `Cerveaux/Cerveau Privé/1 PROJETS/`, lis chaque note de contexte (`[X]/[X].md`).
4. **Casquettes :** liste `Cerveaux/Cerveau Privé/2 CASQUETTES/`, lis chaque note de contexte.

Si le corpus est collectif (étape 3), jette aussi un oeil à l'arborescence des cerveaux partagés présents sous `Cerveaux/` (`ls` sur leurs `1 PROJETS/` et `2 CASQUETTES/`) pour savoir quelles rubriques existent déjà côté équipe. Lecture seule, aucune écriture.
...
RÈGLE ABSOLUE DE DESTINATION
Tout ce que tu routes atterrit dans `Cerveaux/Cerveau Privé/`.
Aucune destination ne commence par un autre cerveau que le Privé,
jamais, quelle que soit l'apparence collective d'un document. Le
partage est une étape ultérieure, faite note par note par un skill
dédié. Toi, tu importes en Privé et tu marques les candidats au
partage dans le champ `partage`.
...
- `source` : chemin relatif depuis la racine du workspace
...
- `destination` : toujours sous `Cerveaux/Cerveau Privé/`
  - `Cerveaux/Cerveau Privé/1 PROJETS/[Projet]/`
  - `Cerveaux/Cerveau Privé/2 CASQUETTES/[Casquette]/`
  - `Cerveaux/Cerveau Privé/3 RESSOURCES/`
  - `Cerveaux/Cerveau Privé/5 ARCHIVE/`
  - `Cerveaux/Cerveau Privé/0 INBOX/` (uniquement si vraiment impossible à router, < 5%)
- `partage` : audience détectée, une **proposition**, pas une action
  - `prive` (défaut, et défaut du doute)
  - le nom d'un cerveau partagé présent sous `Cerveaux/` (voir la règle
    d'audience du `AGENTS.md` racine), quand le contenu est manifestement
    collectif : process, procédure, fiche client, doctrine, doc métier,
    référentiel qu'une équipe doit pouvoir lire. S'il n'existe aucun
    cerveau partagé, cette valeur reste toujours `prive`.
- `partage_reason` : 1 ligne, uniquement si `partage` ≠ `prive`
- `sensible` : true si l'item contient un secret (mot de passe, clé d'API),
  un salaire, des données RH nominatives, des chiffres financiers de l'entreprise,
  ou un jugement sur une personne nommée. Dans ce cas force `partage: prive`.
...
- Pour les conversations Claude/ChatGPT : `EXTRACT_CONTEXT` ou `SUMMARIZE` sont presque toujours mieux que `KEEP_NOTE`. La valeur est dans les idées, pas dans le verbatim. Et elles restent `partage: prive` : un brouillon de pensée ne se partage pas.
- `partage` dans le doute : `prive`. Le partage est irréversible, l'hésitation tranche en faveur du privé.
...
Contrôle avant d'aller plus loin : si une `destination` ne commence pas par `Cerveaux/Cerveau Privé/`, corrige-la en Privé et signale-le. Un sous-agent n'a pas autorité pour écrire ailleurs.
...
Tout atterrit dans le Cerveau Privé. Les candidats au partage sont listés
plus bas : ils seront proposés après l'import, via `/partager`.
...
### KEEP_NOTE → Cerveaux/Cerveau Privé/1 PROJETS/[Projet 1] ([N])
| Source | → Destination | Confidence | Raison |
|---|---|---|---|

### KEEP_NOTE → Cerveaux/Cerveau Privé/2 CASQUETTES/[Casquette] ([N])
[idem]

### KEEP_NOTE → Cerveaux/Cerveau Privé/3 RESSOURCES/ ([N])
[idem]
...
### Candidats au partage ([N]) : proposition, rien n'est déplacé ici
| Note (après import) | Cerveau proposé | Pourquoi |
|---|---|---|
| `1 PROJETS/Onboarding/Process accueil.md` | [cerveau partagé] | procédure, l'équipe doit l'appliquer |
| `1 PROJETS/Cession/Note d'arbitrage.md`   | [cerveau partagé] | sujet du cercle restreint |

> Items marqués `sensible` (secret, salaire, RH, finances) : [N]. Ils restent en
> Privé, ils ne sont pas proposés au partage.
...
Un ajustement peut aussi porter sur la colonne `partage` : l'utilisateur retire ou ajoute un candidat. Ça reste une intention, ça ne déclenche rien à ce stade.
...
Annonce : « Passage en BUILD MODE. J'exécute par lots de 15, tout dans le Cerveau Privé. »
...
Tous ces chemins sont sous `Cerveaux/Cerveau Privé/`. Aucune écriture dans un cerveau partagé pendant cette étape, même pour un item marqué comme candidat au partage.
...
Nettoie les dossiers vides dans `Cerveaux/Cerveau Privé/_Import/` à la fin (sauf `_originaux/` que tu laisses en place pour traçabilité).

---

## Étape 8 bis : Proposer le partage (uniquement s'il y a des candidats)

L'import est fini, tout est rangé en Privé. Tu passes maintenant la deuxième question de routage : qui doit pouvoir lire ça ?

S'il n'y a aucun candidat (`partage: prive` partout), saute cette étape sans rien dire.

S'il y en a :

```
[N] notes importées ressemblent à du collectif :

[Une section par cerveau partagé concerné (les dossiers partagés sous
`Cerveaux/`) :]

Vers le [cerveau partagé] ([X])
- `1 PROJETS/Onboarding/Process accueil.md`  (procédure, l'équipe l'applique)
- `3 RESSOURCES/Clients/Fiche ALPHA.md`      (fiche client, l'équipe en a besoin)
- ...

Elles sont dans ton Cerveau Privé, en sécurité. Rien n'est parti.

Le partage passe par `/partager`, qui lit chaque note et vérifie ce qu'elle
contient avant de la déplacer : secrets, salaires, chiffres financiers, RH.
C'est irréversible, donc ça se fait note par note.

Tu veux :
a) Les passer maintenant en lot, une par une, avec `/partager`
b) Trier d'abord (tu me dis lesquelles tu retires de la liste)
c) Plus tard, on en reste là
```

Si **option a** :

1. Lance `/partager` sur la liste. Le lot est une file d'attente, pas un traitement groupé : `/partager` traite **une note à la fois**, la lit intégralement, rend son verdict de confidentialité, et attend l'arbitrage de l'utilisateur avant de déplacer.
2. Après chaque note : `[i]/[N] traitée. Continue ? (oui / pause / stop)`
3. Une note bloquée par le contrôle de confidentialité reste en Privé. Tu passes à la suivante, tu ne forces jamais.

Tu ne réimplémentes pas le contrôle de confidentialité ici. C'est le job de `/partager`, source unique de cette règle.
...
Résumé :
- KEEP_NOTE       : [X]  → projets / casquettes / ressources (Cerveau Privé)
- EXTRACT_CONTEXT : [Y]  → Moi.md, projets, casquettes enrichis
- SUMMARIZE       : [Z]  → [G] notes de synthèse créées
- ARCHIVE         : [W]  → 5 ARCHIVE/
- Écarté vers `Atelier/` : [A]

Partage :
- Proposées   : [P]
- Déplacées   : [Q]  (détail par cerveau partagé)
- Restées en Privé : [R]  (dont [S] bloquées au contrôle de confidentialité)
...
2. Qu'est-ce que j'aurais pu faire mieux ?
   (poser plus / moins de questions au départ, mieux deviner les verdicts,
    mieux repérer ce qui est collectif, regrouper différemment, mieux
    briefer les sous-agents...)
...
**11. Deux questions de routage, dans l'ordre.** Utile à mémoriser ? Sinon `Atelier/`. Puis : qui doit pouvoir le lire ? C'est ça qui désigne le cerveau.

**12. L'import écrit en Privé, toujours.** Un cerveau partagé ne se remplit que via `/partager`, note par note, après contrôle de confidentialité. Un import de masse qui écrirait direct dans un dossier partagé, c'est une fuite. Dans le doute : Privé, sans même demander.

**13. Une note, un cerveau.** Aucune copie entre cerveaux. Partager, c'est déplacer. Et c'est irréversible : ce que tu proposes au partage, tu le proposes, l'utilisateur tranche.
