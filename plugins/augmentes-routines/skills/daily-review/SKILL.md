...
1. Lis `Cerveaux/Cerveau Privé/2 CASQUETTES/Sur ma vie/Moi.md` pour comprendre l'utilisateur
...
5. Vérifie s'il y a des notes récentes dans `Cerveaux/Cerveau Privé/0 INBOX/`
...
La daily note du jour vit dans `Cerveaux/Cerveau Privé/2 CASQUETTES/Sur ma vie/Life Phases/[Phase active]/YYYY-MM-DD.md`. Format obligatoire : `YYYY-MM-DD.md` (pas de suffixe).

- **Si elle existe déjà** : append la section Daily Review à la fin du fichier, sans toucher au reste (logs `/done`, Focus, Notes...).
- **Si elle n'existe pas** : la créer à partir du template `Cerveaux/Cerveau Privé/4 TOOLS/Templates/Daily Note.md` puis ajouter la section Daily Review à la fin.
